...
#define DIV_FACTOR 0.75 //adjust this factor to adjust the curve smoothness

#define TABLE_CURVE_DIV_FACTOR 0.5
#define DLG_CURVE_DIV_FACTOR 1.0

#define TIME_TABLE_MAX_SIZE 10000
#define table_timenum_min 5
#define dlg_control_point_max 12	//包括两个端点
...
public:
	...
	Curve(float ax,float ay,float bx,float by,float cx,float cy)
	{
		...
		Cx = cx; 
		Cy = cy;
		Ndiv = (max(abs((float)Ax),abs((float)Ay))/(float)DIV_FACTOR);
	}
	Curve() 
	{	
	};

	void PutCurve(float ax,float ay,float bx,float by,float cx,float cy,float div_factor) 
	{
		Ax = ax;
		Ay = ay;
		Bx = bx; 
		...
		Cx = cx; 
		Cy = cy;
		Ndiv = (max(abs((float)Ax),abs((float)Ay))/div_factor);
	}
...
public:
	...
	//	
	//}
	int GetCurveCount(float div_factor)
	{
		Curve c;
		int count = 0;
		for(int i=0; i<NP-1 ; i++) 
		{
			c.PutCurve(Ax[i],Ay[i],Bx[i],By[i],Cx[i],Cy[i],div_factor);
			count += c.GetCount();
		}
		return count;
	}
	void GetCurve(POINT points[], int& PointCount, float div_factor)
	{
		Curve c;
		for(int i=0; i<NP-1 ; i++) 
		{
