...
DWORD CSerialPort::Read(void* lpBuf, DWORD dwCount)
{
	...
	//modify by hzm 2011-03-22
	dwCount = min(comstat.cbInQue, dwCount);
	if (dwCount && !ReadFile(m_hComm, lpBuf, dwCount, &dwBytesRead, NULL))
	{
...
void CSerialPort::CancelIo()
{
	...
	if (!zmSerialPort::_SerialPortData.m_lpfnCancelIo(m_hComm))
	{
